...
contract MultiSigWallet {
    ...
    // owners confirmations for each transaction
    mapping(uint => mapping(address => bool)) public isConfirmed;

    Transaction[] public transactions;

    // modifiers running checks before functions are executed 
    modifier onlyOwner() {
    ...
    function executeTransaction(uint _txIndex) public onlyOwner notExecuted(_txIndex) {
        Transaction storage transaction = transactions[_txIndex];

        require(transaction.numConfirmations >= numConfirmationsRequired, "cannot execute query");
